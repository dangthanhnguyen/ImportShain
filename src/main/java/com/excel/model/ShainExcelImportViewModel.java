...
public class ShainExcelImportViewModel {
    ...
    /**
     * コンストラクタ。ShainRepository、ShainValidator、ExcelReaderを初期化します。
     */
     ...
     * @throws SQLException データベース操作エラーが発生した場合
     */
    private void importInsertOnlyToDatabase(List<ShainDAO> shainList, ImportResult result, Consumer<ImportResult> progressCallback) throws SQLException {
     ...
     * @throws SQLException データベース操作エラーが発生した場合
     */
    private void importInsertAndUpdateToDatabase(List<ShainDAO> shainList, Set<String> existingShainNos, ImportResult result, Consumer<ImportResult> progressCallback) throws SQLException {
        try (Connection conn = DbConnectionUtil.getConnection()) {
            // トランザクション開始：自動コミットを無効化
            conn.setAutoCommit(false);
            try {
     ...
     * @throws SQLException データベース操作エラーが発生した場合
     */
    private void importNewToDatabase(List<ShainDAO> shainList, ImportResult result, Consumer<ImportResult> progressCallback) throws SQLException {
        try (Connection conn = DbConnectionUtil.getConnection()) {
            // トランザクション開始：自動コミットを無効化
            conn.setAutoCommit(false);
            try {
     ...
     * @throws SQLException データベース操作エラーが発生した場合
     */
    private List<ShainDAO> validateAndLoadData(String filePath, List<String> errorMessages, boolean checkExisting) throws IOException, SQLException {
        // ステップ1: ファイルの検証
     ...
     */
    private void classifyShainRecords(List<ShainDAO> shainList, Set<String> existingShainNos, List<ShainDAO> toInsert, List<ShainDAO> toUpdate) {
